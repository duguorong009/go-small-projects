...
func Write(w http.ResponseWriter, cookie http.Cookie) error {
	...
	// return the ErrValueTooLong error if it's more than 4096 bytes.
...
func Read(r *http.Request, name string) (string, error) {
	...
	decodedString, err := base64.URLEncoding.DecodeString(cookie.Value)
	if err != nil {
...
func ReadSigned(r *http.Request, name string, secretKey []byte) (string, error) {
	...
	// a bit more understandable at glance.
	...
	// in the cookie. If they match, we can be confident that the cookie name and
	// value haven't been edited by the client.
...
func ReadEncrypted(r *http.Request, name string, secretKey []byte) (string, error) {
	...
	// uses strings.Cut() to split it on the first ":" character.
