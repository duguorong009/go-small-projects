...
func FindAlbum(id string) (*Album, error) {
	...
	// will have a length of zero. So check fo this and return an
...
func IncrementLikes(id string) error {
	...
	// the reply's Err filed at this point in case there was a problem
	...
	// And we do the same with the increment on our sorted test.
...
func FindTopThree() ([]*Album, error) {
	...
	// Begin an infinite loop. In a real application, you might
	// want to limit this to a set number of attemps, and return an
	// error if the transaction doesn't successfully complete within
	// those attempts.
	for {
		...
		// Execute the transaction. Importantly, use the redis.ErrNill
